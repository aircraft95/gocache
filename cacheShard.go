...
func initNewShard(config Config) *cacheShard {
	return &cacheShard{
...
func (s *cacheShard) set(hashedKey uint32, value []byte) {
	s.lock.Lock()
...
func (s *cacheShard) get(hashedKey uint32) ([]byte, error) {
	s.lock.RLock()
...
func (s *cacheShard) del(hashedKey uint32) (bool, error) {
	s.lock.Lock()
	//读取这个key在valueItems开始的下标
